...
- `ENDS`:对于一个包含`ENDS`子句定义的重复性事件，此列包含相应的[DATETIME](../Chapter_11/11.03.01_The_DATE_DATETIME_and_TIMESTAMP_Types.md)值。相比`EXECUTE_AT`列，此值可以在任何可用的表达式中解析。如果在事件没有`ENDS`子句，此列是`NULL`。
